...
def transform(df: pd.DataFrame, *args, **kwargs):
    """
    Build dimensional tables and a fact table from NYC taxi-like data.
    Returns DataFrames (not dicts) so Mage can serialize them correctly.
    """
    # Ensure datetimes
    df = df.copy()
    ...
    passenger_count_dim['passenger_count_id'] = passenger_count_dim.index.astype('int64')
    ...
    trip_distance_dim['trip_distance_id'] = trip_distance_dim.index.astype('int64')
    ...
    pickup_location_dim['pickup_location_id'] = pickup_location_dim.index.astype('int64')
    ...
    dropoff_location_dim['dropoff_location_id'] = dropoff_location_dim.index.astype('int64')
    ...
    payment_type_dim['payment_type_id'] = payment_type_dim.index.astype('int64')
...
def test_output(output, *args) -> None:
    ...
    assert "fact_table" in output and not output["fact_table"].empty, "fact_table is empty"
